...
- `reference/runtime/jelly-anthropomorphic-player-walk-v3.png` is the current 12-frame walking sprite sheet (four directions × three phases), rebuilt from the preserved original. Its explicit 3×4 cut geometry, recovered RIGHT→UP boundary pixels, and enlarged 420×400 runtime cells are documented in `reference/runtime/jelly-anthropomorphic-player-walk-v3.json`; the walk sequence is `[1, 0, 1, 2]` with frame 1 as the stop frame. v2 remains available for comparison and is not overwritten. `reference/jelly-anthropomorphic-player.png` remains the four-direction fallback, and `reference/generated-npcs-hiker-elder-child-hq.png` contains the updated hiker, elder and child sprites with transparent alpha.
