...
- `reference/runtime/jelly-anthropomorphic-player-walk-v2.png` is the current 12-frame walking sprite sheet (four directions × three phases). Its explicit 3×4 cut geometry is documented in `reference/runtime/jelly-anthropomorphic-player-walk-v2.json`; every output-cell border is exact alpha 0 and the original `reference/jelly-anthropomorphic-player-walk.png` is preserved for comparison. `reference/jelly-anthropomorphic-player.png` remains the four-direction fallback, and `reference/generated-npcs-hiker-elder-child-hq.png` contains the updated hiker, elder and child sprites with transparent alpha.
