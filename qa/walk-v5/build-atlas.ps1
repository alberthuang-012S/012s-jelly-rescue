...
public static class WalkAtlas {
  ...
  public static void Build(string root) {
    ...
    var left=Pieces(root+"/qa/walk-v5/left-source.png").OrderBy(p=>p.X).ToList();
    var right=Pieces(root+"/qa/walk-v5/right-source.png").OrderBy(p=>p.X).ToList();
    ...
    using(var g=Graphics.FromImage(atlas)) {
      ...
      atlas.Save(root+"/reference/runtime/jelly-anthropomorphic-player-walk-v5.png",ImageFormat.Png);
      using(var audit=new Bitmap(1680,1600))using(var a=Graphics.FromImage(audit)){
        ...
        audit.Save(root+"/qa/walk-v5/contact-sheet.png",ImageFormat.Png);
      }
    }
