...
public static class WalkAtlas {
  ...
  public static void Build(string root) {
    ...
    var source=Pieces(root+"/reference/runtime/jelly-anthropomorphic-player-walk-v7.png").OrderBy(p=>p.Y).ToList(); if(source.Count!=12)throw new Exception("Expected twelve v7 poses"); var left=source.Skip(3).Take(3).OrderBy(p=>p.X).ToList();
    var right=source.Skip(6).Take(3).OrderBy(p=>p.X).ToList();
    Console.WriteLine("Components: atlas="+full.Count+" left="+left.Count+" right="+right.Count);
    if(full.Count!=12||left.Count!=3||right.Count!=3)throw new Exception("Unexpected sprite component count");
    ...
    using(var g=Graphics.FromImage(atlas)) {
      ...
      atlas.Save(root+"/reference/runtime/jelly-anthropomorphic-player-walk-v8.png",ImageFormat.Png);
      using(var audit=new Bitmap(1680,1600))using(var a=Graphics.FromImage(audit)){
        ...
        audit.Save(root+"/qa/walk-v8/contact-sheet.png",ImageFormat.Png);
      }
    }
    foreach(var p in full.Concat(source))p.Art.Dispose();
  }
}
"@
