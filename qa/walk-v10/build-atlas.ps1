...
public static class WalkAtlas {
  ...
  public static void Build(string root) {
    ...
    var sides=Pieces(root+"/qa/walk-v10/repair-source.png"); if(sides.Count!=16)throw new Exception("Expected sixteen poses"); var left=sides.OrderBy(p=>p.Y).Skip(4).Take(4).OrderBy(p=>p.X).ToList();
    ...
    using(var g=Graphics.FromImage(atlas)) {
      ...
      atlas.Save(root+"/reference/runtime/jelly-anthropomorphic-player-walk-v10.png",ImageFormat.Png);
      using(var audit=new Bitmap(1680,1600))using(var a=Graphics.FromImage(audit)){
        ...
        audit.Save(root+"/qa/walk-v10/contact-sheet.png",ImageFormat.Png);
      }
    }
