...
public static class WalkAtlas {
  ...
  public static void Build(string root) {
    ...
    var sides=Pieces(root+"/qa/walk-v6/side-source.png"); if(sides.Count!=8)throw new Exception("Expected eight side poses"); var left=sides.OrderBy(p=>p.Y).Take(4).OrderBy(p=>p.X).ToList();
    var right=sides.OrderByDescending(p=>p.Y).Take(4).OrderBy(p=>p.X).ToList();
    ...
    using(var g=Graphics.FromImage(atlas)) {
      ...
      atlas.Save(root+"/reference/runtime/jelly-anthropomorphic-player-walk-v6.png",ImageFormat.Png);
      using(var audit=new Bitmap(1680,1600))using(var a=Graphics.FromImage(audit)){
        ...
        audit.Save(root+"/qa/walk-v6/contact-sheet.png",ImageFormat.Png);
      }
    }
